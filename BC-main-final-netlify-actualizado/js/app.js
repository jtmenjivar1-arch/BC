...
function applySettings() {
  ...
  if (BC.settings.heroImage) {
    ...
    if (firstHero) {
      firstHero.src = BC.settings.heroImage;
    }
  }
...
function getUnitPriceForVariant(product, type, size) {
  ...
  */
  ...
  /*
    BASIC:
    Precios fijos definidos por BlackCat.
    XS, S, M, L, XL = $17.00
    2XL = $19.99
    3XL = $25.99
  */
  ...
  /*
    OVERSIZE:
    S, M, L usan el precio base del admin.
    XL, 2XL y 3XL suben según regla.
  */
  if (selectedType === 'oversize') {
    const base = Number(product.price || product.priceOversize || 19.99);

    if (selectedSize === 'XL') return Math.max(base, 22.99);
    if (selectedSize === '2XL') return Math.max(base, 23.99);
    if (selectedSize === '3XL') return Math.max(base, 28.99);
  ...
  /*
    BOXY FIT:
    S, M, L usan el precio base del admin.
    XL y 2XL suben a $25.00.
  */
  if (selectedType === 'boxyfit') {
    const base = Number(product.price || product.priceBasic || 22.00);

    if (selectedSize === 'XL' || selectedSize === '2XL') {
      return Math.max(base, 25.00);
    }
