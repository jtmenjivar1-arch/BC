  ...
  return (items || [])
    .map((p, index) => {
      ...
      const shirtMode = normalizeMode(
        p.shirt_mode || p.shirtMode || p.product_type || p.productType || 'basic'
      );
      ...
      const colors = normalizeList(
        p.colors || p.colores || p.available_colors || p.color || p.availableColors
      );
...
function getUnitPriceForVariant(product, type, size) {
  ...
  if (type === 'basic') {
    ...
    return Number(product.priceBasic || product.price || 16.99);
  }
  ...
  if (type === 'oversize') {
    ...
    return Number(product.priceOversize || product.price || 19.99);
  }

  if (type === 'croptop') {
    return Number(product.priceBasic || product.price || 12.99);
  }
  ...
  if (type === 'boxyfit') {
    ...
    return Number(product.priceBasic || product.price || 22.0);
  }
