...
function applyHero() {
  ...
  const ig = document.querySelectorAll('a[href*="instagram.com"]');
  ig.forEach(a => a.href = `https://instagram.com/${contacts.instagram}`);
  const tk = document.querySelectorAll('a[href*="tiktok.com"]');
  tk.forEach(a => a.href = `https://tiktok.com/@${(contacts.tiktok || '').replace(/^@/,'')}`);
  const wa = document.querySelectorAll('a[href*="wa.me"], .wa-pill');
  wa.forEach(a => a.href = `https://wa.me/${contacts.whatsapp}`);
  const emailRows = [...document.querySelectorAll('.contact-value')].find(el => el.textContent.includes('@'));
  ...
  grid.innerHTML = filteredProducts().map(p => `
    <article class="panel shirt-card">
      <button class="shirt-media" data-id="${p.id}">
        ...
        <span class="shirt-badge">${p.badge}</span>
      </button>
...
function renderTypeChoices() {
  ...
  wrap.innerHTML = typeOptions.map(opt => `<button class="choice ${state.type === opt.id ? 'active' : ''}" data-type="${opt.id}">${opt.label}</button>`).join('');
  wrap.querySelectorAll('[data-type]').forEach(btn => btn.addEventListener('click', () => {
    ...
    renderTypeChoices(); renderSizeChoices(); updateModalPrice();
  }));
}
function renderSizeChoices() {
  ...
  wrap.querySelectorAll('[data-size]').forEach(btn => btn.addEventListener('click', () => {
    ...
    renderSizeChoices(); updateModalPrice();
  }));
}
...
function renderColorChoices() {
  ...
  wrap.innerHTML = colorOptions.map(color => `<button title="${color.id}" class="choice ${state.color === color.id ? 'active' : ''}" data-color="${color.id}" style="${color.style}"></button>`).join('');
...
function updateModalPrice() {
  ...
  document.getElementById('buyNowBtn').onclick = () => window.open(`https://wa.me/${state.data.contacts.whatsapp}?text=${msg}`, '_blank');
}
function openProduct(id) {
  ...
  renderTypeChoices(); renderSizeChoices(); renderColorChoices(); applyPreviewColor(); updateModalPrice();
...
function addCurrentToCart() {
  ...
  if (existing) existing.qty += state.qty;
    ...
    key, title: p.title, character: p.character, anime: p.anime, image: p.image,
    type: state.type, typeLabel: state.type === 'basic' ? 'Básica' : 'Oversize',
    size: state.size, color: state.color, qty: state.qty, unitPrice
  });
...
function closeCart() {
  ...
  if (!document.getElementById('productModal').classList.contains('show')) document.body.style.overflow = '';
}
function checkoutWhatsApp() {
  if (!state.cart.length) return;
  const total = state.cart.reduce((sum, item) => sum + item.unitPrice * item.qty, 0);
  const lines = state.cart.map(item => `• ${item.title} | Personaje: ${item.character} | Anime: ${item.anime} | ${item.typeLabel} | ${item.size} | ${item.color} | Cantidad: ${item.qty} | Total: ${formatPrice(item.unitPrice * item.qty)}`).join('%0A');
...
document.getElementById('backdrop').addEventListener('click', () => { closeCart(); closeProduct(); });
...
document.getElementById('qtyMinus').addEventListener('click', () => { state.qty = Math.max(1, state.qty - 1); updateModalPrice(); });
document.getElementById('qtyPlus').addEventListener('click', () => { state.qty += 1; updateModalPrice(); });
